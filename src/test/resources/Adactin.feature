Feature: Verifying the login functionality of Adactin
  ...
  Scenario Outline: Successfull login with valid credentials
    ...
    Then click continue
    When User is on enter "<firstname>","<lastname>","<Billingaddress>"
      | cardno           | cardtype | expmon  | expyear | cvv |
      | 4263982640269299 | VISA     | January |    2021 | 123 |
    Then Click BookNow button
    When user is on Booking Confirmation page verify the details
    Then click Logout

    Examples: 
      | username  | password | location  | hotel          | Roomtype | No.of Rooms | check in date | check out date | Adult per Room | Children per Room | firstname | lastname | Billingaddress                                                                    |
      | anonym123 | admin123 | Melbourne | Hotel Sunshine | Deluxe   | 6 - Six     | 29/12/2020    | 31/12/2020     | 3 - Three      | 2 - Two           | Greens    | Tech     | Plot No.19,Balamurugan Garden,OMR Road,Kancheepuram District,Okkiam, Thoraipakkam |
